...
elseif ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    switch ($action) {
       ...
       case 'get_donations':
        ...
        // Action publik (tidak perlu login)
        case 'submit_contact_form':
        case 'subscribe_newsletter':
            // ... (Kode untuk kedua case ini dari jawaban sebelumnya tetap di sini) ...
            break;
            ...
            switch ($action) {
                ...
                case 'add_admin':
                    ...
                    if (empty($name) || !filter_var($email, FILTER_VALIDATE_EMAIL) || empty($password)) {
                        $response['message'] = 'Valid name, email, and password (min 6 chars) are required.';
                        break;
                    }
